...
async function ensureAIReviewer() {
...
Respond with a JSON object (no markdown, just raw JSON) with this exact structure:
{
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "You are an expert app reviewer with deep experience in UI/UX design, software engineering, and product management. Analyze the app based on the provided information and give honest, constructive feedback. Rate each category from 1-10 where 1 is poor and 10 is exceptional. Provide specific, actionable suggestions. Always respond with valid JSON only.",
      },
  ...
  let parsed: { ratings: Record<string, number>; feedback: string; suggestions: string[] };
